...
ofstream outfile ("scratch/RLinWiFi-master-original-queue-size-Media-try1.2/CW_data.csv", fstream::out);
...
Ptr<OpenGymDataContainer> MyGetObservation()
{
    ...
    //std::cout << "MyGetObservation: " << box << endl;
    return box;
...
void recordHistory()
{
    ...
    uint32_t nWifi = NodeList::GetNNodes ()-1; // num de stations ou nodes
    //std::cout << "Num de Stations nWifi: " << nWifi << endl;  
    double norm_value = 0;
    double soma_norm_value= 0;  


    for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End ()-1; ++i) {
      Ptr<Node> node = *i; 
      ...
      // Sem Normalizacao:
      // box->AddValue(value); sem normalizacao usa apenas o num de packets da fila
      
      // Com Normalizacao: GetNPackets() / GetMaxSize()
      norm_value = ((double)value)/((double)queue->GetMaxSize().GetValue());
      //std::cout << "norm_value: " << norm_value << endl;
      
      //SOMATORIO
      soma_norm_value = soma_norm_value + norm_value;
      //std::cout << "Somatorio da Qtdade de pacotes da fila: " << soma_norm_value << endl;
    }
    //MEDIA
      double mediaTxBuffer = (soma_norm_value)/(nWifi);
      //std::cout << "Media da Qtdade de pacotes da fila: " << mediaTxBuffer << endl;

    //ratio = errs / sent;
    //history.push_front(ratio);
    
    //mediaTxBuffer = soma_norm_value / nWifi;
